...
class BaseDataset_MultitaskFaceRecognition(Dataset, ABC):
    def __init__(self, name, config, device, isEval, cfg):
        ...
        self.total_images = 0
        self.image_folder = '_arcface_input'
    ...
    def __getitem__(self, index):
        ...
        images = [Path(self.dataset_root, self.name, self.image_folder, path) for path in images[start_idx:end_idx]]
        assert len(images) == end_idx-start_idx
