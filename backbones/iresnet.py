...
class IBasicBlock_1D(nn.Module):
    expansion = 1
    def __init__(self, inplanes, planes, stride=1, downsample=None,
                 groups=1, base_width=64, dilation=1):
        super(IBasicBlock_1D, self).__init__()
        if groups != 1 or base_width != 64:
            raise ValueError('BasicBlock only supports groups=1 and base_width=64')
        if dilation > 1:
            raise NotImplementedError("Dilation > 1 not supported in BasicBlock")
...
class IResNet_1D(nn.Module):
    ...
    def __init__(self,
                 ...
                 groups=1, width_per_group=64, replace_stride_with_dilation=None, fp16=False):
        super(IResNet_1D, self).__init__()
        self.extra_gflops = 0.0
        self.fp16 = fp16
        self.inplanes = 64
        self.dilation = 1
        ...
        self.prelu = nn.PReLU(self.inplanes)
        self.layer1 = self._make_layer_1d(block, 64, layers[0], stride=2)
        self.layer2 = self._make_layer_1d(block,
                                       128,
                                       layers[1],
                                       stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer_1d(block,
                                       256,
                                       layers[2],
                                       stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer_1d(block,
                                       256,
                                       layers[3],
                                       stride=2,
                                       dilate=replace_stride_with_dilation[2])
    ...
    def _make_layer_1d(self, block, planes, blocks, stride=1, dilate=False):
        downsample = None
        previous_dilation = self.dilation
        if dilate:
            self.dilation *= stride
...
def _iresnet(arch, block, layers, pretrained, progress, **kwargs):
    if arch.endswith('1x512'):
        model = IResNet_1D(block, layers, **kwargs)
    else:
...
def iresnet18_1x512(pretrained=False, progress=True, **kwargs):
    return _iresnet('iresnet18_1x512', IBasicBlock_1D, [2, 2, 2, 2], pretrained,
                    progress, **kwargs)
