# duo
# export CUDA_VISIBLE_DEVICES=0; python debug_face_pairs_dataset_id_style_oversampled.py --file-pairs-id-subj /datasets2/bjgbiesseck/face_recognition/synthetic/dcface_with_pretrained_models/dcface_original_synthetic_ids/dcface_original_10000_synthetic_ids_STYLE_FEATURES_CLUSTERING_FROM_1_CASIA-WebFace-imgs_crops_112x112_STYLE_FEATURES_CLUSTERING-feature=_style-_distance=cosine-nclusters=100/feature=_style/_distance=cosine/nclusters=100/face-pairs-subj-style-by-race_num-samples-per-id=49_num-samples-per-cluster=20.json --corresponding-subj-clusters /datasets2/bjgbiesseck/face_recognition/synthetic/dcface_with_pretrained_models/dcface_original_synthetic_ids/dcface_original_10000_synthetic_ids_STYLE_FEATURES_CLUSTERING_FROM_1_CASIA-WebFace-imgs_crops_112x112_STYLE_FEATURES_CLUSTERING-feature=_style-_distance=cosine-nclusters=100/feature=_style/_distance=cosine/nclusters=100/clusters-data_feature=_style.pt_distance=cosine_nclusters=100.pkl --corresponding-style-clusters /datasets2/1st_frcsyn_wacv2024/datasets/real/1_CASIA-WebFace/imgs_crops_112x112_STYLE_FEATURES_CLUSTERING/feature=_style/_distance=cosine/nclusters=100/clusters-data_feature=_style.pt_distance=cosine_nclusters=100.pkl --stylized-imgs /datasets3/bjgbiesseck/face_recognition/dcface/generated_images/tcdiff_WITH_BFM_e:10_spatial_dim:5_bias:0.0_casia_ir50_09-10_1_EQUALIZED-STYLES_BY-RACE_NCLUSTERS=100_ALLOW-STYLE-REPEAT_part=0-4 --corresp-style-features /datasets3/bjgbiesseck/face_recognition/dcface/generated_images/tcdiff_WITH_BFM_e:10_spatial_dim:5_bias:0.0_casia_ir50_09-10_1_EQUALIZED-STYLES_BY-RACE_NCLUSTERS=100_ALLOW-STYLE-REPEAT_part=0-4_STYLE_FEATURES
...
def parse_args():
    ...
    parser.add_argument('--stylized-imgs', type=str, default='', help='')                 # /datasets3/bjgbiesseck/face_recognition/dcface/generated_images/tcdiff_WITH_BFM_e:10_spatial_dim:5_bias:0.0_casia_ir50_09-10_1_EQUALIZED-STYLES_BY-RACE_NCLUSTERS=100_ALLOW-STYLE-REPEAT_part=1-4
    parser.add_argument('--corresp-style-features', type=str, default='', help='')        # /datasets3/bjgbiesseck/face_recognition/dcface/generated_images/tcdiff_WITH_BFM_e:10_spatial_dim:5_bias:0.0_casia_ir50_09-10_1_EQUALIZED-STYLES_BY-RACE_NCLUSTERS=100_ALLOW-STYLE-REPEAT_part=1-4_STYLE_FEATURES
...
def main(args):
    ...
    num_samples_per_selected_cluster = 20
    ...
    for idx_orig_subj_path, orig_subj_path in enumerate(all_face_pairs_subj_style.keys()):
        ...
        # subj_all_orig_style_ids = np.zeros(49*num_samples_per_selected_cluster, dtype=int)
        subj_all_orig_style_ids = torch.zeros(49*num_samples_per_selected_cluster, dtype=int, device=torch.device('cuda:0'))
        ...
        subj_samples_feats = torch.zeros((49*num_samples_per_selected_cluster,128), dtype=torch.float, device=torch.device('cuda:0'))
        for idx_orig_style_path, orig_style_path in enumerate(orig_styles_paths):
        ...
        if num_styles_at_least_one_hit == 49: total_num_subj_having_all_styles_hit += 1
        total_num_styles_at_least_one_hit += num_styles_at_least_one_hit
        total_mean_num_styles_at_least_one_hit = total_num_styles_at_least_one_hit/(idx_orig_subj_path+1)
        print(f'num_styles_at_least_one_hit:', num_styles_at_least_one_hit)
        print(f'    total_mean_num_styles_at_least_one_hit: {total_mean_num_styles_at_least_one_hit}/49 = {total_mean_num_styles_at_least_one_hit/49.0}')
        print(f'    total_num_subj_having_all_styles_hit: {total_num_subj_having_all_styles_hit}/{(idx_orig_subj_path+1)} = {total_num_subj_having_all_styles_hit/(idx_orig_subj_path+1)}')
